    ...
    def receive_send(self):
        ...
        sensor_val[5] is the value from the left middle sensor """
                    ...
                    if(fastestPathStart == True):
                        ...
                        android_msg = android_message_formatter('EXPLORE', [str(exp.robot.descriptor_1()),
                                                                            str(exp.robot.descriptor_2()),"[" + str(
                                19 - exp.robot.center[0]) + "," + str(exp.robot.center[1]) + "]", exp.robot.direction])
                        ...
                        if ((exp.exploredArea <= 99.67 or np.array_equal(exp.robot.center, START)) and continueExplore):
                            ...
                            android_msg = android_message_formatter("EXPLORE", [str(exp.robot.descriptor_1()),
                                                                            str(exp.robot.descriptor_2()), "[" + str(
                                19 - exp.robot.center[0]) + "," + str(exp.robot.center[1]) + "]",
                                                                            exp.robot.direction])  # delete descriptor 3
                        ...
                        # print 'Time 2: %s s' % (time.time() - time_t)
                            ...
                            android_msg = android_message_formatter('DONE', [str(exp.robot.descriptor_1()),
                                                                             str(exp.robot.descriptor_2()), "[" + str(
                                    19 - exp.robot.center[0]) + "," + str(exp.robot.center[1]) + "]", EAST])
                            time.sleep(0.1)
